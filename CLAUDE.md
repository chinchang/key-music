...
- `VibeTypeApp.swift` — `@main`. Creates a `SettingsModel` and an `AudioEngine(settings:)` as `@StateObject`s, starts the engine in `.onAppear`. Declares three scenes: `WindowGroup` for the main window, `Settings { SettingsView(settings:) }` for the standard ⌘, Settings window, and a `MenuBarExtra` with `SettingsLink`, Enable/Disable toggle, and Quit.
- `Settings/SettingsModel.swift` — `ObservableObject` owning three tunables (`bpm` 60–140, `masterVolume` 0–1, `openAtLogin`) persisted to `UserDefaults` via `didSet`. Init mirrors `SMAppService.mainApp.status` so the login-item toggle reflects the OS truth even if the user changed it in System Settings. `openAtLogin` setter calls `SMAppService.mainApp.register()` / `unregister()` in a do/catch — throws when the binary isn't inside a proper `.app` bundle (e.g. `swift run`), in which case the toggle is inert until the installed DMG is used.
- `Settings/SettingsView.swift` — three-section `Form` (Tempo slider / Audio volume slider / Startup toggle) bound to `SettingsModel`.
...
All scheduling uses absolute sample times from engine start. Do not pass relative/"now + offset" times to `scheduleBuffer(at:)` — the look-ahead scheduler assumes step N always maps to frame `N * samplesPerStep`. Groove's swing/jitter is applied on top of that nominal time via `AudioEngine.groovedTime(forStep:)`, never by shifting the step index itself; `Clock`'s step→frame identity stays pure.

**Runtime BPM changes** go through `SettingsModel.bpm` → `AudioEngine.pendingBPM`, applied at the next bar boundary in `applyPendingBPMAtBar()`: `clock.bpm` is swapped and `scheduledThroughStep` is re-seeded to `Int(currentFrame / new samplesPerStep)` so step N onward maps correctly under the new tempo. This produces a perceptible one-bar seam at the tempo change but no garbled timing afterwards. Already-scheduled buffers (in the ~250 ms look-ahead) continue to play at the old tempo through their scheduled time — that's the "seam".
